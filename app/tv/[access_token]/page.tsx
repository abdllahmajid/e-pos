...
// app/tv/[access_token]/page.tsx
// ── KOREKSI ── Dipindah dari `/tv` global (sesi #20) ke sini (migration 027,
// langkah 5) — satu `access_token` = satu TV, isinya masing-masing sendiri.
// Isi & aturan pemutaran TIDAK berubah dari versi lama, HANYA sumber datanya
// yang beda:
//   - LAMA: `fetchPlayablePromoMedia()` → query `.from("promo_media")`
//     langsung, satu playlist global untuk semua TV.
//   - BARU: `fetchScreenPlaylist(accessToken)` → RPC `get_screen_playlist`
//     (`security definer`, migration 027), token dari segmen URL ini,
//     otomatis terbatas ke media milik TV ini saja. Token salah ATAU TV
//     dinonaktifkan sama-sama menghasilkan playlist kosong (lihat komentar
//     `hasLoaded && !current` di bawah — teks layar tunggu SENGAJA generik,
//     tidak membedakan "TV belum diisi" dari "token ini tidak valid", supaya
//     tidak membocorkan informasi ke siapa pun yang mengetik token asal).
//
// Publik lewat middleware.ts (PUBLIC_PATHS "/tv", `startsWith` jadi otomatis
// mencakup path dinamis ini). Sama seperti /cek-struk: halaman ini TIDAK
// boleh mengasumsikan ada sesi/user, dan sengaja tidak memakai hooks/useAuth
// atau komponen modul internal.
//
// Aturan pemutaran (tidak berubah dari sesi #20):
...
export default function TvScreenPage() {
  // `access_token` selalu string tunggal di sini (bukan array) — segmen
  // route ini bernama `[access_token]`, bukan `[...access_token]` (catch-all).
  ...
  useEffect(() => {
    ...
    async function tick() {
      let ok = false;
      try {
        const next = await fetchScreenPlaylist(accessToken);
        if (cancelled) return;
        ok = true;
    ...
    // `accessToken` datang dari segmen URL, tidak berubah selama komponen ini
    // hidup (navigasi ke token lain me-remount halaman, bukan berganti prop)
    // — sengaja tetap dicantumkan di deps supaya lint tidak menandainya lupa,
    // walau praktiknya cuma jalan sekali.
  }, [accessToken]);
  ...
  return (
    ...
    >
      ...
      {/* Layar tunggu: baik "TV ini belum diisi media aktif" maupun "token
          di URL tidak valid" tampil SAMA (teks statis generik) — sengaja
          tidak dibedakan (lihat catatan header file & migration 027), dan
          `anon` tidak boleh membaca tabel `settings` jadi nama toko tidak
